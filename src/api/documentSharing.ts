...
// Mock flag (set to true for local dev if you want mocked shares)
...
export const documentSharingAPI = {
  ...
  shareDocument: async (
    documentId: number,
    clientId: number
  ): Promise<{ data: SharedDocument | null; error: string | null }> => {
    if (USE_MOCK) {
  ...
  acceptShare: async (
    ...
    if (USE_MOCK) {
      await new Promise((resolve) => setTimeout(resolve, 500));
      return {
        data: { ...mockSharedWithMe[0], id: shareId, status: "accepted" },
        error: null,
      };
    }
    ...
    try {
      const response = await apiClient.post(
        `/api/documents/share/${shareId}/accept/`
      );
      return { data: response.data, error: null };
    } catch (error: any) {
      return {
        data: null,
        error: error.response?.data?.message || "Failed to accept share",
      };
    }
  },
  ...
  declineShare: async (
    ...
    if (USE_MOCK) {
      await new Promise((resolve) => setTimeout(resolve, 500));
      return {
        data: { ...mockSharedWithMe[0], id: shareId, status: "declined" },
        error: null,
      };
    }
    ...
    try {
      const response = await apiClient.post(
        `/api/documents/share/${shareId}/decline/`
      );
      return { data: response.data, error: null };
    } catch (error: any) {
      return {
        data: null,
        error: error.response?.data?.message || "Failed to decline share",
      };
    }
  },

  // Get documents shared by me (lawyer)
  getSharedByMe: async (): Promise<{
    data: SharedDocument[] | null;
    error: string | null;
  }> => {
    if (USE_MOCK) {
      await new Promise((resolve) => setTimeout(resolve, 500));
